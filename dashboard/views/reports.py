...
def render_reports():
    ...
    st.caption(
        f"Homestead Intelligence • "
        f"{len(properties):,} properties • "
        f"{neighborhood_df['Neighborhood'].nunique():,} locations"
    )
    report_text = f"""
...
Average Price: ${price_stats['average']:,.0f}
Median Price: ${price_stats['median']:,.0f}
Average Price/Sqft: ${sqft_stats['average']:,.0f}
...
----------------------------------------
Pricing Insight:
{pricing_insight['dominant_segment']} is the dominant price segment,
accounting for {pricing_insight['percentage']:.1f}% of current listings.

Location Insight:
{location_insight['highest_average_price_area']} has the highest
average listing price at ${location_insight['highest_average_price']:,.0f}.

It also leads in total listed market value at
${location_insight['highest_total_market_value']:,.0f}.

Inventory Insight:
{inventory_insight['dominant_status']} listings represent
{inventory_insight['dominant_percentage']:.1f}% of the current market.


MARKET MOVEMENT
----------------------------------------
New Listings: {movement_insight['new_listings']}
Price Changes: {movement_insight['price_changes']}
Availability Changes: {movement_insight['availability_changes']}
Removed Listings: {movement_insight['removed_listings']}
...
Locations: {neighborhood_df['Neighborhood'].nunique():,}

Generated by Homestead Intelligence
"""

    st.download_button(
        label="📥 Download Market Report",
        data=report_text,
        file_name="homestead_market_report.txt",
        mime="text/plain",
